...
function checkLogin(username, password) {
  ...
  const hardcodedUsers = [{ user: "admin", password: "1234" }];
...
function calculateSummary(tickets) {
  ...
  const totalAmount = tickets.reduce(
    (sum, ticket) => sum + parseInt(ticket.amount),
    0
  );
  ...
  if (tickets.length > 0) {
    ...
    tickets.forEach((ticket) => {
      numberCounts[ticket.lotteryNumber] =
        (numberCounts[ticket.lotteryNumber] || 0) + 1;
    });
